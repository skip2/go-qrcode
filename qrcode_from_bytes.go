...
func EncodeFromBytes(contentBytes []byte, level RecoveryLevel, size int) ([]byte, error) {
	var q *QRCodeFromBytes

	q, err := NewFromBytes(contentBytes, level)

	if err != nil {
		return nil, err
	}

	return q.PNGFromBytes(size)
}
...
func WriteFileFromBytes(contentBytes []byte, level RecoveryLevel, size int, filename string) error {
	var q *QRCodeFromBytes
	...
	return q.WriteFileFromBytes(size, filename)
}
...
// a larger image is silently written. Negative values for size cause a variable
// sized image to be written: See the documentation for Image().
func WriteColorFileFromBytes(contentBytes []byte, level RecoveryLevel, size int, background,
...
// New constructs a QRCode. (From Bytes)
//
//	var q *qrcode.QRCode
...
func NewFromBytes(contentBytes []byte, level RecoveryLevel) (*QRCode, error) {
	...
	for _, t := range encoders {
		...
		encoded, err = encoder.encode(contentBytes)
	...
	if err != nil {
		...
		return nil, errors.New("content bytes too long to encode")
	}
...
func NewWithForcedVersionFromBytes(contentBytes []byte, version int, level RecoveryLevel) (*QRCodeFromBytes, error) {
	var encoder *dataEncoder
	...
	if encoded.Len() > chosenVersion.numDataBits() {
		return nil, fmt.Errorf("Cannot encode QR code: content bytes too large for fixed size QR Code version %d (encoded length is %d bits, maximum length is %d bits)",
			version,
			encoded.Len(),
...
// decoding.
func (q *QRCodeFromBytes) BitmapFromBytes() [][]bool {
	// Build QR code.
	q.encodeFromBytes()
...
func (q *QRCodeFromBytes) ImageFromBytes(size int) image.Image {
	// Build QR code.
	q.encodeFromBytes()
...
// PNG returns the QR Code as a PNG image. (From Bytes)
//
// size is both the image width and height in pixels. If size is too small then
// a larger image is silently returned. Negative values for size cause a
// variable sized image to be returned: See the documentation for Image().
func (q *QRCodeFromBytes) PNGFromBytes(size int) ([]byte, error) {
	img := q.ImageFromBytes(size)
...
func (q *QRCodeFromBytes) WriteFromBytes(size int, out io.Writer) error {
	var png []byte

	png, err := q.PNGFromBytes(size)
...
// a larger image is silently written. Negative values for size cause a
// variable sized image to be written: See the documentation for Image().
func (q *QRCodeFromBytes) WriteFileFromBytes(size int, filename string) error {
	var png []byte
...
func (q *QRCodeFromBytes) encodeFromBytes() {
	numTerminatorBits := q.version.numTerminatorBitsRequired(q.data.Len())

	q.addTerminatorBitsFromBytes(numTerminatorBits)
	q.addPaddingFromBytes()

	encoded := q.encodeBlocksFromBytes()
...
func (q *QRCodeFromBytes) addTerminatorBitsFromBytes(numTerminatorBits int) {
...
func (q *QRCodeFromBytes) encodeBlocksFromBytes() *bitset.Bitset {
	// Split into blocks.
...
func (q *QRCodeFromBytes) addPaddingFromBytes() {
...
func (q *QRCodeFromBytes) ToStringFromBytes(inverseColor bool) string {
	bits := q.BitmapFromBytes()
...
func (q *QRCodeFromBytes) ToSmallStringFromBytes(inverseColor bool) string {
	bits := q.BitmapFromBytes()
	var buf bytes.Buffer
